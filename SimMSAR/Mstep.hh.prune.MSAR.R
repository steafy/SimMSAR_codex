function (data, theta, FB, sigma.diag = FALSE, sigma.equal = FALSE, 
          alpha = 0.01, adjust = c("none", "holm", "hochberg", "hommel", "bonferroni", "BH", "BY", "fdr"),
          max_iter = 100, tol = 1e-6) 
{
  ...
  if (p > 0) {
    for (j in 1:M) {
      ...
      tmp2 = (op_2[, , j] + (A2tmp) %*% op[, , j] %*% 
                t(A2tmp) - ((A2tmp) %*% t(op_1[, , j]) + t((A2tmp) %*% 
                                                             t(op_1[, , j]))))/postmix[j] - tmp %*% t(tmp)
      if (!sigma.diag) {
        Sigma[1:d, 1:d, j] = tmp2[1:d, 1:d]
      }
      else {
        Sigma[1:d, 1:d, j] = diag(diag(tmp2[1:d, 1:d]), 
                                  d)
      }
      for (kp in 1:order) {
        A2[j, kp, ] = A2tmp[1:d, ((kp - 1) * d + 1):(kp * 
                                                       d)]
      }
    }
    if (sigma.equal & M > 1) {
      pi_prob = NHMSAR:::normalise(postmix)
      S = pi_prob[1] * Sigma[1:d, 1:d, 1]
      for (j in 2:M) {
        S = S + pi_prob[j] * Sigma[1:d, 1:d, j]
      }
      for (j in 1:M) {
        Sigma[1:d, 1:d, j] = S
      }
  ...
  if (p > 0) {
    list(A = Atmp, A0 = moy, sigma = sigma, prior = prior, 
         transmat = transmat)
  }
  else {
...
# Pruning und Refitten des Modells
for (iter in 1:max_iter) {
  # Pruning-Schritt
  matrices <- c("A", "A0", "sigma")
  pValues <- psychonetrics:::adjust_p_values(result, alpha = alpha, adjust = adjust, 
                             matrices = matrices, mode = "tested")
  
  nonsig <- !is.na(pValues) & pValues > alpha
  if (!all(is.na(nonsig)) && any(nonsig[!is.na(nonsig)])) {
    if (p > 0) {
      for (j in 1:M) {
        for (kp in 1:order) {
          result$A[[j]][[kp]][nonsig[result$A[[j]][[kp]]]] <- 0
        }
      }
      result$A0[nonsig[result$A0]] <- 0
    }
    for (j in 1:M) {
      result$sigma[[j]][nonsig[result$sigma[[j]]]] <- 0
    }
    
    # Refitten des Modells
    theta_new <- list(A = result$A, A0 = result$A0, sigma = result$sigma,
                      prior = result$prior, transmat = result$transmat)
    FB_new <- NHMSAR:::forwardbackward(data, theta_new)
    result_new <- Recall(data, theta_new, FB_new, sigma.diag, sigma.equal)
    
    # Überprüfen der Konvergenz
    if (max(abs(unlist(result_new) - unlist(result))) < tol) {
      break
    }
