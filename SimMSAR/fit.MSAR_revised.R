...
fit.MSAR_revised <- function (data, theta, MaxIter = 100, eps = 1e-05, verbose = FALSE, 
          ...
          lambda2 = 0.1, a = 3.7, ...) 
{
  ...
  Npar = NULL
  cnt <- 0
  FB <- NHMSAR:::Estep.MSAR(data, theta, covar.trans = covar.trans, 
                   covar.emis = covar.emis)
  loglik = FB$loglik
  previous_loglik <- FB$loglik - 1000
  ll_history = NULL
  converged = NHMSAR:::EM_converged(0, 2 * eps, eps)
  par = NULL
  while (converged[1] == 0 && cnt < MaxIter) {
    ...
    if (label == "HH") {
      if (constraints == FALSE & penalty == FALSE & reduct == 
          FALSE) {
        par = NHMSAR:::Mstep.hh.MSAR(data, theta, FB, sigma.diag = sigma.diag, 
                            sigma.equal = sigma.equal)
      }
      else if (constraints) {
        par = NHMSAR:::Mstep.hh.MSAR.with.constraints(data, theta, 
                                             FB, K = K, d.y = d.y)
        attributes(theta)$n_par = M + M * (M - 1) + 
          2 * M * d
      }
      else if (penalty == "ridge") {
        par = NHMSAR:::Mstep.hh.SCAD.cw.MSAR(data, theta, FB, 
                                    penalty = "ridge", lambda1 = 0, lambda2 = lambda2, 
                                    par = par)
      }
      else if (penalty == "LASSO") {
        if (cnt > 1) {
          par = Mstep.hh.reduct.MSAR_patched(data, theta, FB, 
          ...
          # call PATCHED !!!
          #
          debug(NHMSAR:::Mstep.hh.lasso.MSAR)
          par = Mstep.hh.lasso.MSAR_patched(data, theta, FB)
          ret__ = par
        }
      }
      else if (penalty == "SCAD") {
        par = NHMSAR:::Mstep.hh.SCAD.MSAR(data, theta, FB, penalty = "SCAD", 
      ...
      par = NHMSAR:::Mstep.hn.MSAR(data, theta, FB, covar = covar.emis, 
                          verbose = verbose)
      if (order > 0) {
        theta = list(par$A, par$A0, par$sigma, par$prior, 
      ...
      par = NHMSAR:::Mstep.nh.MSAR(data, theta, FB, covar = covar.trans, 
      ...
      par = NHMSAR:::Mstep.nn.MSAR(data, theta, FB, covar.emis = covar.emis, 
    ...
    converged = NHMSAR:::EM_converged(loglik, previous_loglik, eps)
    previous_loglik = loglik
    attributes(theta) = att.theta
    theta = as.thetaMSAR_revised(theta, label = label, ncov.emis = ncov.emis, 
                         ncov.trans = ncov.trans)
    FB = NHMSAR:::Estep.MSAR(data, theta, covar.emis = covar.emis, 
  ...
  if (M > 1) {
    ...
    theta = as.thetaMSAR_revised(theta, label = label, regime_names = NULL, ncov.emis = ncov.emis, 
                         ncov.trans = ncov.trans)
