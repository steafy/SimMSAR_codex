...
Mstep.hh.lasso_glm.MSAR_patched <- function (data, theta, FB) 
{
  T = dim(data)[1]
  ...
  for (j in 1:M) {
    ...
    Cxy = postmix[j] * op_1[, , j] - m[, j] %*% t(m_1[, 
                                                      j])
    for (id in 1:d) {
      glmnet.1 = glmnet(wx[, , j], wy[, id, j], alpha = 1)
      coef = coef(glmnet.1)
      BIC.lm = NULL
      mylm = list()
      w = list()
      
      for (kst in 1:length(glmnet.1$lambda)) {
        w[[kst]] = which(abs(coef[, kst][-1]) > 1e-05)
