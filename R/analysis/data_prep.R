...
# -----------------------------------------------------------------------------
# estimate_MSAR() now returns only raw matrices (orig_A/est_A,
# orig_K, orig_Sigma/est_Sigma, orig_AC). This function turns those
# into the scored outcomes the rest of the pipeline expects, so that every
# scoring decision (edge threshold, K condition-number cutoff, AC horizon,
...
#                  (0.05 in the driver). Below it, |entries| -> 0. A: full
#                  matrix; K: off-diagonal only (the dominant diagonal is
#                  structural and never thresholded).
#   AC_HORIZON     finite horizon T_ac for average_controllability(); MUST equal
#                  generation's default (25) or AC_corr compares two
#                  different definitions of controllability.
#   K_COND_MAX max acceptable condition number of est_Sigma. Above it, the
#                  inversion to K is flagged invalid and ALL K quantities
#                  for that regime become NA -- per regime, never dropping the
#                  whole fit, and never affecting A. Start near the old 1e6;
...
#                  A/K entries that are tens to tens-of-thousands of times
...
#                  K_COND_MAX -- but has uniformly tiny eigenvalues, so its
#                  inverse still explodes). Because NRMSE is an *absolute*
#                  squared-error measure, a single such entry can inflate it by
#                  orders of magnitude (observed max: NRMSE_A = 84.7,
#                  NRMSE_K = 8423). NRMSE is the most SENSITIVE derived
...
#                  whenever EITHER side is magnitude-flagged: A (NRMSE_A,
#                  A_corr, A_sen, A_spec, AC_corr_pearson,
#                  AC_corr_spearman) AND K (NRMSE_K, K_corr,
#                  K_sen, K_spec). The two sides are CROSS-COUPLED (a
#                  A flag NAs K too, and vice versa) because they are not
#                  independent failures: sigma[[j]]/K for a regime are built
#                  directly from that regime's A M-step residuals (tmp2 = op_2
#                  + A2.lasso %*% op %*% t(A2.lasso) - ...), so a magnitude-
#                  exploded A propagates into its Sigma/K; and both
...
#                  flagged rows have markedly lower A_corr AND K_corr and
...
compute_recovery_metrics <- function(MSAR_dynamics_list,
  ...
  threshold_A <- function(mat) {
    mat[abs(mat) < min_edg_val] <- 0
    mat
  }
  threshold_K_offdiag <- function(mat) {
  ...
  est_K    <- vector("list", n)
  est_AC  <- vector("list", n)
  A_corr    <- numeric(n)
  K_corr   <- numeric(n)
  A_sen     <- numeric(n); A_spec  <- numeric(n)
  K_sen    <- numeric(n); K_spec <- numeric(n)
  AC_corr_pearson  <- numeric(n)
  AC_corr_spearman <- numeric(n)
  NRMSE_A   <- numeric(n)
  NRMSE_K  <- numeric(n)
  ...
  for (r in seq_len(n)) {
    orig_A    <- MSAR_dynamics_list$orig_A[[r]]
    est_A_raw <- MSAR_dynamics_list$est_A[[r]]
    orig_K   <- MSAR_dynamics_list$orig_K[[r]]
    est_Sigma    <- MSAR_dynamics_list$est_Sigma[[r]]
    orig_AC <- MSAR_dynamics_list$orig_AC[[r]]

    # --- Unmatched-true-regime guard (partial regime match) ---------------------
    # estimate_MSAR() emits rows with est_A / est_Sigma = NULL for a true
    # regime whose estimated counterpart was a degenerate (zero-variance) A and
    ...
    # outcome (A AND K) to NA and log the row -- do NOT call
    # threshold_A()/cor()/solve() on a NULL/NA. Distinct from the sigma and
    # magnitude gates: those flag a computed-but-untrustworthy estimate, whereas
    # here no estimate exists at all.
    if (is_missing_estimate(est_A_raw)) {
      est_K[[r]]   <- NA
      est_AC[[r]] <- NA
      A_corr[r]  <- NA_real_
      A_sen[r]   <- NA_real_; A_spec[r]  <- NA_real_
      K_corr[r] <- NA_real_
      K_sen[r]  <- NA_real_; K_spec[r] <- NA_real_
      AC_corr_pearson[r]  <- NA_real_
      AC_corr_spearman[r] <- NA_real_
      NRMSE_A[r]  <- NA_real_
      NRMSE_K[r] <- NA_real_
    ...
    # --- A (unaffected by Sigma conditioning): threshold, then score ---------
    est_A <- threshold_A(est_A_raw)

    A_corr[r] <- cor(as.vector(orig_A), as.vector(est_A), method = "pearson")
    bss          <- senspec(orig_A, est_A)
    A_sen[r]  <- bss[["sensitivity"]]
    A_spec[r] <- bss[["specificity"]]
    NRMSE_A[r] <- rmse_true_edges(as.vector(orig_A), as.vector(est_A)) / EDGE_VALUE_RANGE

    # Average controllability from the thresholded est_A (deterministic).
    # Computed BEFORE the plausibility gate so that all A-derived metrics
    ...
    ac <- average_controllability(est_A, T_ac = AC_HORIZON)
    est_AC[[r]] <- ac
    AC_corr_pearson[r]  <- cor(orig_AC, ac, method = "pearson")
    AC_corr_spearman[r] <- cor(orig_AC, ac, method = "spearman")
    ...
    # can inflate NRMSE_A by orders of magnitude. Here we only FLAG and log the
    # regime; the actual NA-ing is deferred to the cross-coupled block after the
    # K metrics are computed, because a magnitude-exploded A propagates into
    # the SAME regime's Sigma/K (sigma[[j]] is built from A's M-step
    ...
    A_flagged  <- FALSE
    K_flagged <- FALSE
    max_abs_A <- max(abs(est_A))
    if (!is.finite(max_abs_A) || max_abs_A > max_plausible_magnitude) {
      A_flagged <- TRUE
      magnitude_records[[length(magnitude_records) + 1L]] <- tibble::tibble(
        ...
        ts_id     = MSAR_dynamics_list$ts_id[r],
        regime_id = MSAR_dynamics_list$regime_id[r],
        reason    = "A_implausible_magnitude",
        max_abs_value = max_abs_A
      )
    }

    # --- K: invert est_Sigma, gate on condition number ----------------------
    ...
    if (is.null(inv)) {
      ...
      if (!is.finite(rc) || rc <= 1 / K_COND_MAX) {
    ...
    if (is.na(invalid_reason)) {
      kap <- threshold_K_offdiag(symmetrize_matrix(inv))
      est_K[[r]] <- kap
      K_corr[r] <- cor(vectorize_upper_tri(orig_K, diag = FALSE),
                           vectorize_upper_tri(kap,        diag = FALSE),
                           method = "pearson")
      kss           <- senspec_upper_tri(orig_K, kap, diag = FALSE)
      K_sen[r]  <- kss[["sensitivity"]]
      K_spec[r] <- kss[["specificity"]]
      NRMSE_K[r] <- rmse_true_edges(vectorize_upper_tri(orig_K, diag = FALSE),
                                        vectorize_upper_tri(kap,        diag = FALSE)) / EDGE_VALUE_RANGE

      # Plausibility gate, part 1 -- DETECT (mirrors the A one above). A
      # Sigma_hat can be well-conditioned BY RATIO (passes K_COND_MAX via
      # rcond) while having uniformly tiny eigenvalues, so its inverse still
      # explodes in absolute terms. Off-diagonal only, matching what NRMSE_K
      # scores (the diagonal is a different, unbounded inverse-variance scale and
      # is never thresholded either -- see threshold_K_offdiag()). As with
      # A we only FLAG here; the NA-ing (of BOTH sides) is deferred to the
      # cross-coupled block after this if/else, since the two explosions co-occur.
      off_mask <- !diag(TRUE, nrow(kap))
      max_abs_K <- max(abs(kap[off_mask]))
      if (!is.finite(max_abs_K) || max_abs_K > max_plausible_magnitude) {
        K_flagged <- TRUE
        magnitude_records[[length(magnitude_records) + 1L]] <- tibble::tibble(
          timesteps = MSAR_dynamics_list$timesteps[r],
          density   = MSAR_dynamics_list$density[r],
          ...
          ts_id     = MSAR_dynamics_list$ts_id[r],
          regime_id = MSAR_dynamics_list$regime_id[r],
          reason    = "K_implausible_magnitude",
          max_abs_value = max_abs_K
        )
      }
    } else {
      est_K[[r]] <- NA
    ...
    # exist. A magnitude explosion on either side (A_flagged OR K_flagged)
    # invalidates ALL derived outcomes of that regime row -- BOTH the A side
    # (A_corr, A_sen, A_spec, AC_corr_pearson/spearman, NRMSE_A)
    # AND the K side (K_corr, K_sen, K_spec, NRMSE_K) -- not
    # just the flagged side. Rationale: sigma[[j]]/K are computed from that
    # regime's A M-step residuals, so a magnitude-degenerate A propagates
    # into its Sigma/K; and both explosions share a root cause (very low
    ...
    # within the same design cell, flagged rows have far lower A_corr/K_corr
    # and much smaller postmix than unflagged rows. (K may already be NA here
    # via the condition-number gate; the assignment is idempotent in that case.)
    if (isTRUE(A_flagged) || isTRUE(K_flagged)) {
      A_corr[r]             <- NA_real_
      A_sen[r]              <- NA_real_
      A_spec[r]             <- NA_real_
      AC_corr_pearson[r]  <- NA_real_
  ...
  MSAR_dynamics_list$est_K   <- est_K
  MSAR_dynamics_list$est_AC <- est_AC
  MSAR_dynamics_list$A_corr   <- A_corr
  MSAR_dynamics_list$K_corr  <- K_corr
  MSAR_dynamics_list$A_sen    <- A_sen
  MSAR_dynamics_list$A_spec   <- A_spec
  MSAR_dynamics_list$K_sen   <- K_sen
  MSAR_dynamics_list$K_spec  <- K_spec
  MSAR_dynamics_list$AC_corr_pearson  <- AC_corr_pearson
  MSAR_dynamics_list$AC_corr_spearman <- AC_corr_spearman
  MSAR_dynamics_list$NRMSE_A  <- NRMSE_A
  MSAR_dynamics_list$NRMSE_K <- NRMSE_K
  ...
  cat("K metrics (K_corr, NRMSE_K, K_sen/spec) set to NA for those rows;\n")
  cat("A metrics for the same rows are unaffected. See summarize_sigma_validity().\n")
  ...
  cat("cross-coupled: EITHER an A_ or K_implausible_magnitude flag NAs A\n")
  cat("(NRMSE_A, A_corr, A_sen/spec, AC_corr_pearson/spearman) AND\n")
  cat("K (NRMSE_K, K_corr, K_sen/spec) for that regime. Per regime,\n")
  ...
  if (n_unmatched > 0) {
    ...
    cat("All derived metrics (A + K) NA for these rows -- the true regime's\n")
    cat("estimated counterpart was a degenerate zero-variance A and was left\n")
...
# regime's K quantities (not the whole fit, not A).
...
# was for A in the first place, but the thresholded estimate still contains
...
# EITHER an A_ or K_implausible_magnitude flag NAs A (NRMSE_A,
# A_corr, A_sen/spec, AC_corr_pearson/spearman) AND K (NRMSE_K,
# K_corr, K_sen/spec) for that regime -- because the two explosions
# co-occur (K is built from A's M-step residuals; shared low-postmix root
...
prepare_corr_results <- function(MSAR_dynamics_list) {
  ...
  # Drives reliability of A_corr/K_corr (a Pearson r computed from very
  ...
  # from the already-stored orig_A/orig_K list-columns; no re-estimation
  # needed. orig_K can be NA (excluded near-singular Sigma_hat fits), hence
  # NA_integer_ fallback rather than letting sum()/sapply() error.
  corr_results <- corr_results %>%
    mutate(
      n_nz_A = sapply(orig_A, \(m) sum(m != 0)),
      n_nz_K = sapply(orig_K, \(m) {
        if (length(m) == 1 && is.na(m)) return(NA_integer_)
        sum(vectorize_upper_tri(m, diag = FALSE) != 0)
      })
    )
  
  # NRMSE_A / NRMSE_K, A_corr/K_corr, sens/spec and the AC
  # correlations are now computed upstream in compute_recovery_metrics() (from
  # the raw stored matrices, with the edge threshold and the K
