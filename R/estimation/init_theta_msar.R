...
# Adaptation: renamed to init_theta_msar(); logic unchanged apart from formatting.
# -----------------------------------------------------------------------------
...
# Initialise an NHMSAR theta parameter object (means, AR coefficients, residual
# covariances, transition matrix) for M regimes at a given AR order. Adapted from
# the NHMSAR package.
init_theta_msar <- function (data, ..., M, order, regime_names = NULL, nh.emissions = NULL,
                                     ...
                                     verbose = FALSE) 
{
  if (missing(M) || is.null(M) || M==0) {warnig("Need at least one regime : M=1"); M <- 1 }
  ...
  if (M>1) {
    ...
    transmat <- NHMSAR:::mk_stochastic(diag(1,M)+matrix(runif(M*M),M)) # homogeneous Markov chain
  } else {
    prior = 1
    transmat = 1
  } 
