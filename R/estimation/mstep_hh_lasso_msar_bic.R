...
#
# LEGACY "bic" engine: the lars-subset + in-sample-BIC + WLS-refit first M-step
# (BIC length/regime bug fixed, see docs/MSTEP_LASSO_BIC_BUG.md). This is the
# DEFAULT first M-step used by fit_msar (penalty="LASSO") -- selecting a nearly
# saturated support that the downstream min_edg_val threshold prunes. The
# genuinely-penalized cv.glmnet engine + per-iteration re-selection lives in
# mstep_hh_lasso_msar.R and is opt-in via options (see that file / the CV doc).
