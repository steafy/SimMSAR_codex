...
init.theta.MSAR_revised <- function (data, ..., M, order, regime_names = NULL, nh.emissions = NULL, 
                                     ...
                                     cl.init = "mean") 
{
  ...
  if (missing(M) || is.null(M) || M == 0) {
    print("Need at least one regime: M=1")
    M <- 1
  }
  if (missing(order)) {
    order <- 0
  ...
  d = dim(data)[3]
  print(paste("T:", T, "N.samples:", N.samples, "d:", d))
  
  if (cl.init == "mean" | order == 0) {
    d.data = matrix(0, (T - 1) * N.samples, d)
    data.mat = matrix(0, T * N.samples, d)
    print("Dimensions of d.data:")
    print(dim(d.data))
    print("Dimensions of data.mat:")
    print(dim(data.mat))
    
    for (id in 1:d) {
      d.data[, id] = c(data[2:T, , id, drop = FALSE] - data[1:(T - 1), , id, drop = FALSE])
      data.mat[, id] = c(data[, , id, drop = FALSE])
    }
    ...
    if (M > 1) {
      if (order > 0) {
        ...
        if (any(duplicated(centers))) {
          ...
          if (any(duplicated(centers))) {
            print("Is there a lot of equal data?")
          }
        }
  ...
  if (order > 0) {
    res = NHMSAR:::Mstep.classif(data, array(class, c(T, N.samples, 1)), order = order)
    A = res$A
    A0 = res$A0
    sigma = res$sigma
  } else {
  ...
  if (substr(label, 1, 1) == "N") {
    ...
    if (missing(nh.transitions)) {
      nh.transition = "gauss"
    }
    if (!is.function(nh.transitions)) {
      if (nh.transitions == "gauss") {
        nh.transitions = function(covar, par.trans, transmat) {
          ...
          for (j in 1:M) {
            for (nc in 1:ncov) {
              p.t[, , nc] = par.trans[j, nc]
            }
            xx = (covar - p.t)^2
            sxx = apply(xx, 1, sum)
            temp = exp(-0.5 * sxx / par.trans[j, ncov + 1]^2) / par.trans[j, ncov + 1]
            for (i in 1:M) {
        ...
        nh.transitions = function(covar, par.trans, transmat) {
          ...
          ncov = dim(covar)[3]
          M = dim(transmat)[1]
          par.trans = repmat(t(par.trans[, 2]) * exp((0+1i) * 
                                                       par.trans[, 1]), M, 1) %*% (matrix(1, M, 
                                                                                          M) - diag(1, nrow = M))
          f = array(0, c(M, M, T))
          for (j in 1:M) {
            for (i in 1:M) {
              f[i, j, ] = transmat[i, j] * abs(exp(par.trans[i, j] * exp(-(0+1i) * covar)))
            }
          }
          f.sum = apply(f, c(1, 3), sum)
          for (i in 1:M) {
  ...
  if (order == 0 && label == "HH") {
    theta = list(A0, sigma, prior, transmat)
  }
  if (order == 0 && label == "HN") {
    theta = list(A0, sigma, prior, transmat, par.emis)
  }
  if (order == 0 &&  label == "NH") {
    theta = list(A0, sigma, prior, transmat, par.trans)
  }
  if (order == 0 && label == "NN") {
    theta = list(A0, sigma, prior, transmat, par.trans, par.emis)
  }
  else if (order > 0 && label == "HH") {
    theta = list(A, A0, sigma, prior, transmat)
  }
  else if (order > 0 && label == "HN") {
    theta = list(A, A0, sigma, prior, transmat, par.emis)
  }
  else if (order > 0 && label == "NH") {
    theta = list(A, A0, sigma, prior, transmat, par.trans)
  }
  else if (order > 0 && label == "NN") {
    theta = list(A, A0, sigma, prior, transmat, par.trans, par.emis)
  }
  attr(theta, "NbComp") <- d
  ...
  # trace(as.thetaMSAR_revised, tracer = browser)
  theta = as.thetaMSAR_revised(theta, label = label, ncov.emis = ncov.emis, ncov.trans = ncov.trans)
  return(theta)
}
